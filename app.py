        ...
        for account_key, account_data in data.items():
            ...
            if 'regions' in account_data:
                for region_name, region_data in account_data['regions'].items():
                    ...
                    # Use dict to merge customers/environments with different case
                    customers_merged = {}

                    if 'customers' in region_data:
                        for customer_name, customer_data in region_data['customers'].items():
                            # Normalize customer name to uppercase for case-insensitive grouping
                            customer_key = customer_name.upper()

                            if customer_key not in customers_merged:
                                customers_merged[customer_key] = {
                                    'name': customer_key,
                                    'type': 'customer',
                                    'environments': {}
                                }
                            ...
                            if 'environments' in customer_data:
                                for env_name, env_data in customer_data['environments'].items():
                                    # Normalize environment name to uppercase for case-insensitive grouping
                                    env_key = env_name.upper()

                                    if env_key not in customers_merged[customer_key]['environments']:
                                        customers_merged[customer_key]['environments'][env_key] = {
                                            'name': env_key,
                                            'type': 'environment',
                                            'instances': []
                                        }
                                    ...
                                    if 'instances' in env_data:
                                        for instance in env_data['instances']:
                                            ...
                                            customers_merged[customer_key]['environments'][env_key]['instances'].append(instance_node)

                    # Convert merged dict to list format
                    for customer_key, customer_node in customers_merged.items():
                        customer_node['environments'] = list(customer_node['environments'].values())
...
def get_scan_status():
    ...
    return jsonify({
        ...
        'cache_age': int(cache_age) if cache_age != float('inf') else -1,
...
def get_aws_profiles():
    ...
    try:
        if os.path.exists(credentials_path):
            ...
            # Get all section names (profiles)
            profiles = list(config.sections())
            
            # Filter out any empty sections
            profiles = [profile for profile in profiles if profile.strip()]
            
            if profiles:
                print(f"Found AWS profiles from {credentials_path}: {profiles}")
            else:
                print(f"No profiles found in {credentials_path}")
                # Fallback to default profile if credentials file exists but has no sections
                profiles = ['default']
        else:
...
def organize_instances_by_tags(instances, tag1_name, tag2_name):
    ...
    for instance in instances:
        ...
        # Normalize tag values to uppercase for case-insensitive grouping
        tag1_value = instance.get(tag1_name.lower(), 'Unknown').upper()
        tag2_value = instance.get(tag2_name.lower(), 'Unknown').upper()
...
def launch_rdp_client():
    ...
    try:
        ...
        def build_command(client_name, local_port, username=None, password=None):
            base_commands = {
                'darwin': {
                    'MacFreeRDP': ['open', '-n', '-a', 'MacFreeRDP', '--args', '/cert:ignore', f'/v:127.0.0.1:{local_port}', '/w:1920', '/h:1080', f'/t:{instance_name}'],
                ...
                'linux': {
                    'FreeRDP (xfreerdp)': ['xfreerdp', f'/v:127.0.0.1:{local_port}', f'/cert:ignore', f'/title:{instance_name}'],
...
# =============================================================================
# Guacamole RDP Support for Docker
# =============================================================================

# Check if running in Docker mode with Guacamole
def is_guacamole_mode():
    """Check if Guacamole mode is enabled (for Docker deployment)"""
    return os.environ.get('RDP_MODE', 'native') == 'guacamole'


def get_ssm_forwarder_url():
    """Get the URL for the SSM forwarder service"""
    host = os.environ.get('SSM_FORWARDER_HOST', 'ssm-forwarder')
    port = os.environ.get('SSM_FORWARDER_PORT', '5001')
    return f'http://{host}:{port}'


def generate_guacamole_token(connection_params):
    """
    Generate an encrypted token for guacamole-lite.
    The token format expected by guacamole-lite is:
    - JSON object with 'iv' and 'value' fields (both base64 encoded)
    - The whole JSON is then base64 encoded
    """
    from Crypto.Cipher import AES
    from Crypto.Util.Padding import pad
    import os as crypto_os

    # Secret key (must match guacamole-lite server) - must be exactly 32 bytes for AES-256
    secret_key = os.environ.get('GUAC_CRYPT_SECRET', 'cloudterm-guac-secret-key-32byte')
    ...
    json_data = json.dumps(connection_params).encode()
    encrypted = cipher.encrypt(pad(json_data, AES.block_size))

    # Create the token object in guacamole-lite format
    token_data = {
        'iv': base64.b64encode(iv).decode('ascii'),
        'value': base64.b64encode(encrypted).decode('ascii')
    }

    # Base64 encode the JSON token object (use separators to remove spaces for compact JSON)
    token_json = json.dumps(token_data, separators=(',', ':'))
    token = base64.b64encode(token_json.encode()).decode()

    print(f"[DEBUG] Generated token length: {len(token)}")
    print(f"[DEBUG] Token JSON: {token_json[:100]}...")

    return token


@app.route('/rdp-client')
def rdp_client_page():
    """
    Serve the RDP client HTML page for browser-based RDP sessions.
    """
    return render_template('rdp-client.html')


@app.route('/rdp-mode', methods=['GET'])
def get_rdp_mode():
    """
    Get the current RDP mode (native or guacamole)
    """
    return jsonify({
        'mode': 'guacamole' if is_guacamole_mode() else 'native',
        'guacamole_url': os.environ.get('GUACAMOLE_URL', '') if is_guacamole_mode() else None
    })


@app.route('/start-guacamole-rdp', methods=['POST'])
def start_guacamole_rdp():
    """
    Start a Guacamole-based RDP session for Docker deployment.

    This endpoint:
    1. Requests the SSM forwarder to establish port forwarding to the Windows instance
    2. Returns a URL that can be opened in a new browser tab for the Guacamole RDP session
    """
    try:
        data = request.get_json()
        instance_id = data.get('instance_id')
        ...
        # Parse domain from username if provided in DOMAIN\user or user@domain format
        domain = ''
        if username:
            if '\\' in username:
                # DOMAIN\user format
                ...
                domain = parts[0]
                username = parts[1]
            elif '@' in username and '.' not in username.split('@')[-1]:
                # user@DOMAIN format (not email format like user@domain.com)
                parts = username.split('@', 1)
                username = parts[0]
                domain = parts[1]
        ...
        # Request the SSM forwarder to establish port forwarding
        ssm_forwarder_url = get_ssm_forwarder_url()

        try:
            response = requests.post(
                f'{ssm_forwarder_url}/start',
                json={
                    'instance_id': instance_id,
                    'instance_name': instance_name,
                    'aws_profile': aws_profile,
            ...
            if response.status_code != 200:
                return jsonify({
                    'success': False,
                    'error': f'SSM forwarder error: {response.json().get("error", "Unknown error")}'
                })

            forwarder_result = response.json()

            if not forwarder_result.get('success'):
                return jsonify({
                    'success': False,
                    'error': forwarder_result.get('error', 'Failed to establish port forwarding')
                })

            # Get the forwarded port info
            rdp_host = forwarder_result.get('host', 'ssm-forwarder')
            rdp_port = forwarder_result.get('local_port')

        except requests.exceptions.RequestException as e:
            return jsonify({
                'success': False,
                'error': f'Cannot connect to SSM forwarder service: {str(e)}'
            })

        # Build guacamole-lite connection parameters
        # Note: Modern Windows servers typically require NLA (Network Level Authentication)
        settings = {
            'hostname': rdp_host,
            'port': str(rdp_port),
        ...
        # Add domain if provided (required for domain accounts)
        if domain:
        ...
        token = generate_guacamole_token(connection_params)

        # Build the RDP client URL with encrypted token (credentials are hidden)
        guac_ws_url = os.environ.get('GUAC_WS_URL', 'ws://localhost:8080')
        rdp_client_url = f'/rdp-client?token={quote(token, safe="")}&name={quote(instance_name, safe="")}&id={quote(instance_id, safe="")}&ws={quote(guac_ws_url, safe="")}'
        ...
        return jsonify({
            ...
            'rdp_host': rdp_host,
            'rdp_port': rdp_port,
            'rdp_client_url': rdp_client_url,
            'message': 'RDP session ready. Opening in new browser tab...'
        })

    except Exception as e:
        print(f"Error starting Guacamole RDP session: {str(e)}")
        return jsonify({'success': False, 'error': str(e)})


@app.route('/stop-guacamole-rdp', methods=['POST'])
def stop_guacamole_rdp():
    """
    Stop a Guacamole-based RDP session
    """
    try:
        data = request.get_json()
        instance_id = data.get('instance_id')
        ...
        # Request the SSM forwarder to stop the session
        ssm_forwarder_url = get_ssm_forwarder_url()

        try:
            response = requests.post(
                f'{ssm_forwarder_url}/stop',
            ...
            if response.status_code == 200:
                ...
                return jsonify({
                    'success': False,
                    'error': response.json().get('error', 'Failed to stop session')
                })
        ...
        print(f"Error stopping Guacamole RDP session: {str(e)}")
        return jsonify({'success': False, 'error': str(e)})


@app.route('/guacamole-sessions', methods=['GET'])
def list_guacamole_sessions():
    """
    List all active Guacamole RDP sessions
    """
    try:
        ssm_forwarder_url = get_ssm_forwarder_url()

        response = requests.get(f'{ssm_forwarder_url}/sessions', timeout=10)

        if response.status_code == 200:
            return jsonify(response.json())
        else:
            return jsonify({'sessions': [], 'error': 'Failed to fetch sessions'})

    except requests.exceptions.RequestException as e:
        return jsonify({'sessions': [], 'error': f'SSM forwarder unavailable: {str(e)}'})
